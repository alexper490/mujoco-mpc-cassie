...
class GenerateProtoGrpcCommand(setuptools.Command):
  """Specialized setup command to handle filter proto compilation.

  Generates the `filter_pb2{_grpc}.py` files from `filter_proto`. Assumes that
  ...
  def run(self):
    """Generate `filter.proto` into `filter_pb2{_grpc}.py`.
    ...
    for the generated `filter_pb2_grpc.py` file. The final argument of the
    `protoc` call has to be "mujoco_mpc/filter.proto" in order for the import to
    become `from mujoco_mpc import [filter_pb2_proto_import]` instead of just
    `import [filter_pb2_proto_import]`. The latter would fail because the name is
    ...
    proto_module_relative_path = Path(
        "mujoco_mpc", "proto", filter_proto_filename
    )
    filter_proto_destination_path = Path(
        build_lib_path, proto_module_relative_path
    )
    filter_proto_destination_path.parent.mkdir(parents=True, exist_ok=True)
    # Copy `filter_proto_filename` into current source.
    shutil.copy(filter_proto_source_path, filter_proto_destination_path)
    ...
    protoc_command_parts = [
        ...
        str(filter_proto_destination_path),
    ]
...
class CopyFilterServerBinaryCommand(setuptools.Command):
  """Specialized setup command to copy `filter_server` next to `filter.py`.

  Assumes that the C++ gRPC `filter_server` binary has been manually built and
  and located in the default `mujoco_mpc/build/bin` folder.
  """

  description = "Copy `filter_server` next to `filter.py`."
  user_options = []
...
class CopyTaskAssetsCommand(setuptools.Command):
  """Copies `filter_server` and `ui_filter_server` next to `filter.py`.
...
class BuildPyCommand(build_py.build_py):
  """Specialized Python builder to handle filter service dependencies.

  During build, this will generate the `filter_pb2{_grpc}.py` files and copy
  `filter_server` binary next to `filter.py`.
  """
...
class BuildCMakeExtension(build_ext.build_ext):
  ...
  def run(self):
    self._configure_and_build_filter_server()
    self.run_command("copy_filter_server_binary")
  ...
  def _configure_and_build_filter_server(self):
    ...
    print("Building `filter_server` and `ui_filter_server` with CMake")
    subprocess.check_call(
        [
            cmake_command,
            "--build",
...
setuptools.setup(
    ...
    cmdclass={
        ...
        "copy_filter_server_binary": CopyFilterServerBinaryCommand,
